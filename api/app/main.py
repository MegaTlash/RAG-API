...
@app.post("/querywithscreenshot", response_model=QueryResponse)
async def upload_screenshot_to_model(query: str = Form(...), file: UploadFile = File(...)):
    valid_ext = ["png", "jpg", "jpeg"]
    if file.filename.split('.')[-1].lower() not in valid_ext:
        raise HTTPException(status_code=400, detail="Not a valid image file (png, jpg, jpeg) was uploaded. ")
    ...
    rag.append_message({"role": "user", "content": query})
    ...
    if rag.rag_enabled and rag.retrieval_pipeline:
        try:
            docs = rag.retrieve_documents(query, OLLAMA_API_URL, MODEL, chat_history)
            context = "\n".join(
    ...
    system_prompt = create_system_prompt(chat_history, context, query)
    response = get_response_ollama(system_prompt, images=[encoded_image])
    try:
